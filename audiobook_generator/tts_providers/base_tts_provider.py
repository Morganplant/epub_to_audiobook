...
TTS_OPENAI = "openai"
TTS_EDGE = "edge"
TTS_PARLER = "parler"  # Added for Parler TTS
...
def get_supported_tts_providers() -> List[str]:
    return [TTS_AZURE, TTS_OPENAI, TTS_EDGE, TTS_PARLER]  # Added TTS_PARLER
...
def get_tts_provider(config) -> BaseTTSProvider:
    ...
    elif config.tts == TTS_PARLER:  # Added for Parler TTS
        from audiobook_generator.tts_providers.parler_tts_provider import ParlerTTSProvider
        return ParlerTTSProvider(config)
    else:
